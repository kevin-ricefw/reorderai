"""Normalize product names for inventory Γåö vendor catalog matching and similar-item grouping."""
...
# Multi-word brands removed before token brand strip (order matters: longer first)
...
def expand_aliases(text: str) -> str:
    ...
    # Strip multi-word brands first so "DAILY DELIGHT DRUMSTICK" → "DRUMSTICK"
...
def normalize_product_name(name: str, *, size_field: str | None = None) -> str:
    """Alphanumeric key after alias expansion ΓÇö for exact-ish comparisons."""
...
def product_signature(name: str, *, size_field: str | None = None) -> dict[str, Any]:
    ...
    # Singularize common plurals so DRUMSTICKS ≈ DRUMSTICK
    normed: list[str] = []
...
def match_score(a: str, b: str, *, size_a: str | None = None, size_b: str | None = None) -> float:
    """
    Score 0ΓÇô100 for inventory description vs vendor catalog name.
    Requires size match when both sides have a parseable size ΓÇö unless core
    ...
    # Last resort string similarity ΓÇö only when tokens are weak/short
